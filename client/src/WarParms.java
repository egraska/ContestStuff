...
public class WarParms extends JFrame implements ActionListener {
	...
	public WarParms (String ttl, int type, Project pr) {
		...
		r.add(new JLabel("     repeatedly click an asset "));
		r.add(new JLabel(" "));
		...
		label.setFont(new Font("Helvetica", Font.PLAIN, 26));
		...
		add("Center", keyboard = new Keyboard(project));
		...
		q.setBackground(bkgcolor);
		q.add(new JLabel(" "));		
		q.add(new JLabel("I declare war with player", JLabel.RIGHT));
		p.add(q);
		...
		// for (int i=0 ; i < type ; i++) g[i].setText("20"); // <- for testing only
	}
	...
	synchronized public void actionPerformed (ActionEvent evt) {
		if (evt.getSource() == help) {
			if (project.mousehelp != null) project.mousehelp.war_window();
			return;
		...
		if (evt.getSource() == cancel) {
			...
			String tmp = "";
			if (keyboard != null) tmp = keyboard.msg.getText();
         ...
         out += " " + tmp + " ";
			...
			tmp = "";
			for (int i=0 ; i < type ; i++) {
				if (str[i].equalsIgnoreCase("Weapons") && t[i].getText().length() > 0) {
					tmp += "WEAPONS "+t[i].getText();
					break;
				}
			}
			...
			tmp = "";
			for (int i=0 ; i < type ; i++) {
				if (str[i].equalsIgnoreCase("Vehicles") && t[i].getText().length() > 0) {
					tmp += "VEHICLES "+t[i].getText();
					break;
				}
			}
			if (tmp == null || tmp.equals("")) {
